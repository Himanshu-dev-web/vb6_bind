...
Begin VB.Form frmTitle 
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.ComboBox cboInner_Cover 
         ...
         EndProperty
         Height          =   360
         ItemData        =   "frmTitle.frx":0000
         Left            =   60
         List            =   "frmTitle.frx":0019
         Sorted          =   -1  'True
   ...
   Begin VSFlex7Ctl.VSFlexGrid vs 
      ...
      FormatString    =   $"frmTitle.frx":004D
      ScrollTrack     =   0   'False
      ScrollBars      =   2
      ScrollTips      =   0   'False
      MergeCells      =   0
...
If MsgBox("Want To Delete ?", vbQuestion + vbYesNo) = vbYes Then
   con.Execute "delete from Title where EntryNo =" & txtEntryNo.Text & ""
   Call cmdRef_Click
End If
...
txtEntryNo.Text = MaxSNo("Title", "EntryNo")
...
'
'If rs.State = 1 Then rs.Close
'rs.Open "select * from Title  where ChallanNo ='" & txtChallan & "'", con
'If rs.EOF = False Then
...
If rs.State = 1 Then rs.Close
rs.Open "select * from Title where EntryNo=" & txtEntryNo.Text & "", con, adOpenDynamic, adLockOptimistic
If rs.EOF = True Then
   rs.AddNew
End If
...
If ss = "R" Then
    ...
    If cboBook.Text = "" Then
       f.Open "select * from Title where Issue_Rec='R' order by EntryNo", con, adOpenDynamic, adLockOptimistic
    Else
       f.Open "select * from Title where Issue_Rec='R' and bookname='" & cboBook.Text & "' order by EntryNo", con, adOpenDynamic, adLockOptimistic
    End If
...
Else
    ...
    If cboCustomer.Text = "" Then
       f.Open "select * from Title where Issue_Rec='D' order by EntryNo", con, adOpenDynamic, adLockOptimistic
    Else
       f.Open "select * from Title where bookname ='" & cboBook & "' and Issue_Rec='D' order by EntryNo", con, adOpenDynamic, adLockOptimistic
    End If
...
If ss = "R" Then
lblHead = "TITLE/INNER/PAPER RECEIVED : "
''lblBinder.Visible = True
''cboCustomer.Visible = True
Else
lblHead = "TITLE CONSUMED : "
''
...
If f.State = 1 Then f.Close
f.Open "select * from Title where EntryNo =" & vs.TextMatrix(vs.RowSel, 0) & "", con, adOpenDynamic, adLockOptimistic
If f.EOF = False Then
cmdDel.Enabled = True
