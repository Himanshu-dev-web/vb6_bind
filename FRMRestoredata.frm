...
If LCase(L1(Index).Caption) = LCase("quit") Then
Unload Me
Exit Sub
End If
...
Sub ReStoreData()
    ...
    If MsgBox("This action will remove the present data." & vbCrLf & "Do you want to continue 'Yes' OR 'No' ?", vbQuestion + vbYesNo, "Warning") = vbYes Then
        ...
        Dim rfile As New FileSystemObject
        If rfile.FileExists(Replace(DirR.Path & "\Data.mdb", "\\", "\")) = False Then
        ...
        rfile.CopyFile Replace(DirR.Path & "\Data.mdb", "\\", "\"), MDB_Path, True
        Set nfol1 = rfile.GetFile(MDB_Path)
        nfol1.Attributes = 0
