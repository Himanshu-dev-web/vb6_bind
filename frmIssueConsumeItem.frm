...
Private Sub cmdadd_Click()
 If rs.State = 1 Then rs.Close
 rs.Open "select distinct(IssueNo) from IssueConsumeItem where IssueDate >=datevalue('" & FromDate.Value & "') and IssueDate <=datevalue('" & ToDate.Value & "') and ReturnQty =False order by IssueNo", con
 ListHeatingNo.Clear
 If rs.EOF = False Then
    While rs.EOF = False
...
Sub SearchData()
    ...
    If rs.State = 1 Then rs.Close
    rs.Open "select * from IssueConsumeItem where IssueNo='" & n & "' and ReturnQty = False", con
...
popuplist2 "Select IssueNo,IssueDate from IssueConsumeItem where ReturnQty = False group by IssueNo,IssueDate", con
