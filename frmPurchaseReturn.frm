...
Begin VB.Form frmPurchaseReturn 
   ...
   Begin VSFlex7Ctl.VSFlexGrid vs 
      ...
      FormatString    =   $"frmPurchaseReturn.frx":0000
      ScrollTrack     =   0   'False
      ScrollBars      =   2
      ScrollTips      =   0   'False
      MergeCells      =   0
...
Private Sub cmdadd_Click()
   ...
   rs.Open "select distinct(MRVNo) from purchaseReturn where (RecDate>=datevalue('" & FromDate.Value & "') and RecDate<=datevalue('" & ToDate.Value & "')) order by MRVNo", con, adOpenDynamic, adLockOptimistic
   ListBillNo.Clear
...
Private Sub cmdBill_Click()
   ...
   If txtParty.Text <> "" Then
      cr.ReplaceSelectionFormula "{purchaseReturn.RecDate}>=datevalue('" & Format(FromDate.Value, "MM/dd/yyyy") & "') and {purchaseReturn.RecDate}<=datevalue('" & Format(ToDate.Value, "MM/dd/yyyy") & "') and {purchaseReturn.PartyName}= '" & txtParty.Text & "'"
   Else
      cr.ReplaceSelectionFormula "{purchaseReturn.RecDate}>=datevalue('" & Format(FromDate.Value, "MM/dd/yyyy") & "') and {purchaseReturn.RecDate}<=datevalue('" & Format(ToDate.Value, "MM/dd/yyyy") & "')"
   End If
...
Sub DeleteData()
    ...
    cmd.CommandText = "delete from purchaseReturn where BillNo='" & txtBillNo.Text & "'"
    cmd.Execute
...
Sub SaveData()
    ...
    str1 = ""
    If rs.State = 1 Then rs.Close
    rs.Open "select * from purchaseReturn", con, adOpenDynamic, adLockOptimistic
...
'        str1 = Trim(vs.TextMatrix(i, 1)) & Space(26 - a) & Trim(vs.TextMatrix(i, 2)) & Space(7 - Len(Str(Trim(vs.TextMatrix(i, 2))))) & Trim(vs.TextMatrix(i, 3)) & Space(7 - Len(vs.TextMatrix(i, 3))) & vs.TextMatrix(i, 4) & Space(7 - Len(vs.TextMatrix(i, 4))) & vs.TextMatrix(i, 5)
   ...
   Next
    ...
    'con.Execute "update purchaseReturn set TempData='" & str1 & "' where MRVNo='" & Me.txtMrvNo.Text & "'"
    str1 = ""
...
'
'            If rs1.State = 1 Then rs1.Close
'            rs1.Open "select * from purchaseReturn where ItemName='" & vs.TextMatrix(i, 0) & "' and BillNo='" & txtBillNo.Text & "'", con, adOpenDynamic, adLockOptimistic
'
...
Private Sub cmdSave_Click()
    ...
    If rs.State = 1 Then rs.Close
    rs.Open "select * from purchaseReturn where MRVNo='" & txtMrvNo.Text & "'", con, adOpenDynamic, adLockOptimistic
    If rs.EOF = True Then
...
Sub Search()
    ...
    rs_search.Open "select * from purchaseReturn where MRVNo='" & txtMrvNo.Text & "'", con, adOpenDynamic, adLockOptimistic
...
If KeyCode = 113 Then
popuplist2 "SELECT DISTINCT mrvno,partyname,billno,billdate from purchaseReturn", con
End If
End Sub
