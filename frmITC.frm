...
Begin VB.Form frmITC 
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.ComboBox cboInner_Cover 
         ...
         EndProperty
         Height          =   336
         ItemData        =   "frmITC.frx":0000
         Left            =   60
         List            =   "frmITC.frx":0022
         Sorted          =   -1  'True
...
Private Sub cboBook_Click()
 ...
 If rs.State = 1 Then rs.Close
 rs.Open "select inner_paper_cover from itc where BookName='" & cboBook.Text & "' and len(inner_paper_cover)>0 and Issue_Rec='R' group by inner_paper_cover", con
 While rs.EOF = False
 
 If rs1.State = 1 Then rs1.Close
 rs1.Open "select sum(Qty) from itc where bookname='" & cboBook.Text & "' and inner_paper_cover='" & rs(0) & "'", con
 If Not IsNull(rs1(0)) Then
    
  If lblData.Caption = "" Then
    lblData.Caption = rs(0) & " : " & rs1(0)
  Else
    lblData.Caption = lblData.Caption & vbCrLf & rs(0) & " : " & rs1(0)
  End If
...
' If rs.State = 1 Then rs.Close
' rs.Open "select inner_paper_cover from itc where BookName='" & cboBook.Text & "' and Issue_Rec='D' group by inner_paper_cover", con
' While rs.EOF = False
'
' If rs1.State = 1 Then rs1.Close
' rs1.Open "select sum(Qty) from itc where bookname='" & cboBook.Text & "' and  Issue_Rec='D'", con
' If Not IsNull(rs1(0)) Then
'
'  If lblData.Caption = "" Then
'    lblData.Caption = rs(0) & " : " & rs1(0)
...
If cboInner_Cover.Visible = True Then
  cboInner_Cover.SetFocus   'txtChallan.SetFocus
Else
 txtChallan.SetFocus
End If
...
If MsgBox("Want To Delete ?", vbQuestion + vbYesNo) = vbYes Then
   con.Execute "delete from ITC where EntryNo =" & txtEntryNo.Text & ""
   Call cmdRef_Click
End If
...
txtEntryNo.Text = MaxSNo("ITC", "EntryNo")
...
'
'If rs.State = 1 Then rs.Close
'rs.Open "select * from ITC  where ChallanNo ='" & txtChallan & "'", con
'If rs.EOF = False Then
...
If cboInner_Cover = "" Then
   MsgBox "Select Inner/Paper/Cover ...", vbCritical
   Exit Sub
...
If rs.State = 1 Then rs.Close
rs.Open "select * from ITC where EntryNo=" & txtEntryNo.Text & "", con, adOpenDynamic, adLockOptimistic
If rs.EOF = True Then
   rs.AddNew
End If
...
If ss = "R" Then
    ...
    If cboBook = "" Then
       f.Open "select * from ITC where Issue_Rec='R' order by EntryNo", con, adOpenDynamic, adLockOptimistic
    Else
       f.Open "select * from ITC where Issue_Rec='R' and bookname='" & cboBook.Text & "' order by EntryNo", con, adOpenDynamic, adLockOptimistic
    End If
...
vs.TextMatrix(i, 6) = f!inner_paper_cover
...
vs.FormatString = "StNo|RecDate|<BookName|Qty|ChallanNo|Description|Inner/Cover/Paper"
...
Else
    ...
    If cboBook.Text = "" Then
       f.Open "select * from ITC where Issue_Rec='D' order by EntryNo", con, adOpenDynamic, adLockOptimistic
    Else
       f.Open "select * from ITC where bookname ='" & cboBook & "' and Issue_Rec='D' order by EntryNo", con, adOpenDynamic, adLockOptimistic
    End If
...
If f.State = 1 Then f.Close
f.Open "select * from ITC where EntryNo =" & vs.TextMatrix(vs.RowSel, 0) & "", con, adOpenDynamic, adLockOptimistic
If f.EOF = False Then
cmdDel.Enabled = True
...
If Not IsNull(f!inner_paper_cover) Then
